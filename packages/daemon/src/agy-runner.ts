...
export const DEFAULT_REMOTE_HANDS_REMINDER =
  '[Context Reminder: Remote Hands autonomous control plane. ZERO DISCOVERY STEPS: Never run `which rh`, `rh --help`, `rh profiles`, or explore flags. Take immediate action on the user\'s request. The desktop browser is already running and authenticated with the user\'s personal profile. For browser actions, use `rh browser open "<url>"`, `rh browser snapshot`, `rh browser click <index>`, `rh browser type <index> "<text>"`, or `browser-harness <<\'PY\' ... PY`. For sensitive actions (posting, deleting, deploying), run `rh approve "<summary>" --action=publish`. If rejected, read the user\'s rejection reason in stderr, modify the content as requested, and re-request approval or cancel if directed. When human approval is granted (`[HUMAN APPROVAL GRANTED]`), immediately execute the action (click Post / Submit) without asking for approval again. All actions stream live to the phone. Provide a clean, direct final summary.]';

...
export const DEFAULT_REMOTE_HANDS_SYSTEM_PROMPT =
  ...
  '4. Mandatory Human Approval & Rejection Handling: Before executing sensitive or irreversible actions (e.g. posting to social media, deleting files or repositories, sending messages, deploying to production, or spending money), you MUST run `rh approve "<summary>" [--risk=high] [--action=publish|delete|push|pay|send]`. The summary MUST describe the exact proposed content or action (e.g. `rh approve "Post tweet: Check out this repository: https://github.com/..." --action=publish --risk=high`). Only proceed if it exits with code 0. If rejected by the user, `rh approve` exits with code 1 and writes the user\'s rejection reason to stderr (`Approval rejected by user: <reason>`). Read this rejection feedback carefully: modify the proposed content or action according to the user\'s instructions and re-run `rh approve`, or cancel if the user asked to abort. When human approval is granted (indicated by `[HUMAN APPROVAL GRANTED]`), the user has already approved your proposed action on their mobile phone: execute the action immediately (e.g. click Post / Submit) without asking for approval again.\n' +
  '5. Live Screen Streaming: Every action and page state is captured and streamed live to the user\'s phone in real time.\n' +
  '6. Decisive Completion: Once the task is completed or verified, provide a clean, concise markdown summary of what was accomplished.\n' +
...
export const DEFAULT_REMOTE_HANDS_REMINDER =
  '[Context Reminder: Remote Hands autonomous control plane. ZERO DISCOVERY STEPS: Never run `which rh`, `rh --help`, `rh desktop --help`, `rh profiles`, or explore flags. Take immediate action on the user\'s request. The desktop browser is already running and authenticated with the user\'s personal profile. For browser actions, use `rh browser open "<url>"`, `rh browser snapshot`, `rh browser click <index>`, `rh browser type <index> "<text>"`, or `browser-harness <<\'PY\' ... PY`. For native desktop software and windows, use `rh desktop open "<app>"`, `rh desktop window focus "<app>"`, `rh desktop snapshot`, `rh desktop act "<goal>"`, `rh desktop click <index>`, `rh desktop type "<text>"`, `rh desktop key <combo>`, `rh desktop menu "<app>" "<menu>" "<item>"`. For sensitive actions (posting, deleting, deploying), run `rh approve "<summary>" --action=publish`. If rejected, read the user\'s rejection reason in stderr, modify the content as requested, and re-request approval or cancel if directed. When human approval is granted (`[HUMAN APPROVAL GRANTED]`), immediately execute the action (click Post / Submit) without asking for approval again. All actions stream live to the phone. Provide a clean, direct final summary.]';
...
export class ProcessAgentRunner implements AgentRunner {
  ...
  async run(
    ...
    const isContinuation = task.prompt.startsWith('[HUMAN APPROVAL');
    let effectiveTask: Task = { ...task };
    if (!isContinuation) {
